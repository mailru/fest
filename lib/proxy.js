...
function compile(file, srcPath, options){
	...
	try {
		...
		return	compile_error_tpl(file, srcPath+file + ' -- template not found', options);
	}
	...
	try {
		...
		console.log(err.message);
		return	compile_error_tpl(file, 'fest.compile failed', options);
	}
...
function compile_error_tpl(file, txt, options){
