...
if (cmd[0]) {
	...
	config = extend(config, JSON.parse(fs.readFileSync(fn)));
	config.templateDir = path.resolve(root, config.templateDir);
	if (config.compileDir) {
		config.compileDir = path.resolve(root, config.compileDir);
	}
	if (config.localeDir) {
...
http.createServer(function (req, res) {
	...
	if (file) {
		...
		if (config.compileDir) {
			...
			var fn = config.compileDir + '/' + file + (locale ? '.' + locale + '.js' : '.js');
			try {
		...
		if (!result) {
			...
			result = compile(file, locale, config.templateDir, config.compile);
			mtime = new Date;
...
function compile(file, locale, dir, options){
...
function compile_tmpl(file, source, options){
	return fest.compile_tmpl(file, source, options.wrapper);
}
