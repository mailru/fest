...
function help() {
	...
	console.info([
		...
		'',
		'Options:',
		'  --out         output directory for compiled files (use "-" for STDOUT, default)',
...
function abs_path(f) {
	...
	f = f.replace(/^~\//, process.env.HOME + '/');  // don't forget about HOME: '~'
	return path.resolve(process.env.PWD, f);  // convert any path to absolute (relative to current work dir)
}
...
template_file_abs = abs_path(template_file);
...
var language;
if (cmd.translate) {
	translate_file = abs_path(cmd.translate);
