...
function compileAttributes(attrs){
    var i, value, result = [''];
    function fest(value, options){
        options = options || {};
        if (options.doget){
            return {doget:value.name, params:value.params || ''};
        }
        options.output = options.output || 'html';
        if(!('safe' in options)) options.safe = false;
        if(!('check' in options)) options.check = true;

        return {value: value, options:options};
    }
    for (i in attrs){
        value = attrs[i].value;
        if (value.indexOf('fest:') === 0){
            value = 'fest(\'' + value.replace(/^fest\:/, '').replace(/'/g, "\\'") + '\')';
        }
        if (value.indexOf('fest-asis:') === 0){
            value = 'fest(\'' + value.replace(/^fest-asis\:/, '').replace(/'/g, "\\'") + '\', {safe:true, output:\'text\', check:false})';
        }
        if (value.indexOf('fest-get:') === 0){
            value = 'fest(' + value.replace(/^fest-get\:/, '') + ', {doget:true})';
        }
        if (value.indexOf('fest') === 0){
            result.push({name: i, fest: new Function('fest', 'return ' + value)(fest)});
            result.push('');
        } else {
            result.push(result.pop() + ' ' + i + '=\\"');
            result.push(result.pop() + attrs[i].value + '\\"');
        }
    }
    return result;
}

function compileNode(node, context_name){
    var result = '', attrs = compileAttributes(node.attributes);
    if (attrs.some(function(item){return typeof item !== 'string';})){
        result += '__fest_str+="<' + node.name + '";';
        attrs.forEach(function(attr){
            if (typeof attr === 'string'){
                result += '__fest_str+="' + attr + '";';
            } else if (attr.fest.doget) {
                result += '__fest_str+=" ' + attr.name + '=\\"";';
                result += '__fest_result[__fest_result.length]=__fest_str;';
                result += '__fest_str="";';
                result += '__fest_result[__fest_result.length]=function(' + context_name + ', params){';
                result += 'return function(){return ("' + attr.fest.doget + '" in __fest_blocks) ? __fest_blocks' + getName(attr.fest.doget) + '(' + context_name + ', params) : [];};';
                result += '};';
                if (attr.fest.params){
                    result += 'try{' + attr.fest.params + ';}catch(e){__fest_params={};__fest_error(e.message);}';
                } else {
                    result += '__fest_params={};';
                }
                result += '__fest_result[__fest_result.length - 1]=__fest_result[__fest_result.length - 1](' + context_name + ',__fest_params);';
                result += '__fest_str+="\\"";';
            } else if (attr.fest.options.safe && !attr.fest.options.check && attr.fest.options.output === 'text'){
                result += '__fest_str+=" '  + attr.name + '=\\"" + ' + attr.fest.value + ' + "\\"";';
            } else {
                if (!attr.fest.options.safe){
                    result += 'try{__fest_if=' + attr.fest.value + '}catch(e){__fest_if=false;__fest_error(e.message);}';
                } else {
                    result += '__fest_if=' + attr.fest.value + ';';
                }
                if (attr.fest.options.check){
                    result += 'if(__fest_if !== false){';
                }
                result += '__fest_str+=" ' + attr.name + '=\\"";';
                if (attr.fest.options.output === 'html'){
                    result += '__fest_str+=__fest_escapeHTML(__fest_if);';
                } else {
                    result += '__fest_str+=__fest_if;';
                }
                result += '__fest_str+="\\"";';
                if (attr.fest.options.check){
                    result += '}';
        ...
        if (node.name in short_tags){
            result += '__fest_str+="<' + node.name + attrs.join('') + '/>";';
        } else {
            result += '__fest_str+="<' + node.name + attrs.join('') + '>";';
        }
    }
...
function _compile (file, context_name, callbacks, strict, options){
    ...
    parser.onopentag = function (node) {
        ...
        if (node.ns[node.prefix] != fest_ns){
            fn[fn.length - 1] += compileNode(node, context_name);
            return;
        }
...
module.exports = function(file, strict, options){
    ...
    compile.then(function(template){
        ...
        template = 'function(__fest_context){"use strict";' +
            ...
            '__fest_blocks={},__fest_params,__fest_error = (typeof console !== "undefined" ? console.error : function(e){__fest_html += e.message}),' +
            '__fest_htmlchars=/[&<>\\"]/g,' +
            '__fest_htmlhash={\'&\':\'&amp;\',\'<\':\'&lt;\',\'>\':\'&gt;\',\'"\':\'&quot;\'},' +
            '__fest_jschars=/[\\\'\\"\\/\\n\\t\\b\\f]/g,' +
