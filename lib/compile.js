var compile = (function(){
    ...
    function errorMessage(msg, badLine, file) {
        ...
        for (var i = badLine - before; i <= badLine + after; i++) {
            if (lines[i] !== undefined) num.push(i);
        }
        ...
        num.forEach(function(n) {
            badPlace.push(('%n%: ' + lines[n].slice(minWhitespace))
                    .replace('%n%', zeroPadding(n + 1, longest)));
        });
    ...
    function _compile (file, context_name, callbacks, options, cb){
        ...
        parser.onopentag = function (node) {
            ...
            if (node.ns[node.prefix] != fest_ns){
                ...
                opentag = true;
                fn[fn.length - 1] += '__fest_pushstr("<' + node.name + compileAttributes(node.attributes) + '");';
                return;
            }
            ...
            switch (node.local){
                case 'template':
                    ...
                    return;
                case 'doctype':
                    fn[fn.length - 1] += '__fest_pushstr("<!DOCTYPE ");';
                    return;
                case 'attribute':
                    fn[fn.length - 1] += '__fest_pushstr(" ' + _getAttr(node, 'name') + '=\\"");';
                    return;
                case 'comment':
                    fn[fn.length - 1] += '__fest_pushstr("<!--");';
                    return;
                case 'cdata':
                    fn[fn.length - 1] += '__fest_pushstr("<![CDATA[");';
                    return;
                case 'text':
                    fn[fn.length - 1] += node.attributes.value ? '__fest_pushstr("' + _getAttr(node, 'value') + '");' : '';
                    return;
                case 'space':
                    fn[fn.length - 1] += '__fest_pushstr(" ");';
                    return;
                ...
                case 'value':
                    ...
                    if (node.__fest_output === 'text'){
                        ...
                        fn[fn.length - 1] += '__fest_pushstr(__fest_escapeJS(';
                    } else {
                        fn[fn.length - 1] += '__fest_pushstr(__fest_escapeHTML(';
                    }
                    return;
                case 'script':
                    if (!node.attributes.safe){
                    ...
                    return;
                case 'insert':
                    fn[fn.length - 1] += '__fest_pushstr("' + readFileSync(dirname(file) + '/' + _getAttr(node, 'src'), 'utf-8')
                        .replace(/\\/g, '\\\\')
                        .replace(/"/g, '\\"')
                        .replace(/\n/g, '\\n')
                        .replace(/\r/g, '') + '");';
                ...
                case 'set':
                    ...
                    fn[fn.length - 1] += 'if (typeof document === "undefined"){var document = {write:function(string){__fest_pushstr(string);}};}';
                    fn[fn.length - 1] += 'var __fest_result=[],__fest_str="",__fest_pushstr;';
                    fn[fn.length - 1] += 'if(typeof __fest_push === "undefined"){__fest_pushstr = function(str){__fest_str+=str}}';
                    return;
                case 'get':
        ...
        parser.onclosetag = function () {
            ...
            if (node.ns[node.prefix] != fest_ns){
                if (!(node.name in short_tags)){
                    fn[fn.length - 1] += '__fest_pushstr("</' + node.name + '>");';
                }
                return;
            }
            switch (node.local){
        ...
        parser.ontext = function (text) {
            ...
            if (evallist.indexOf(stack[stack.length - 1]) === -1){
                fn[fn.length - 1] += '__fest_pushstr("' + text.replace(/\\/g, '\\\\').replace(/\n/g, '\\n').replace(/\r/g, '').replace(/"/g, '\\"') + '");';
        ...
        parser.oncdata = function (cdata) {
            ...
            if (evallist.indexOf(stack[stack.length - 1]) === -1){
                fn[fn.length - 1] += '__fest_pushstr("' + cdata.replace(/\\/g, '\\\\').replace(/\n/g, "\\n").replace(/\r/g, '').replace(/"/g, '\\"') + '");';
            } else if (stack[stack.length - 1] === 'get') {
    ...
    function finish_compile(file, options, name){
        ...
        function build_template(result) {
            ...
            template = 'function ' + name + '(__fest_context){"use strict";' +
                ...
                '__fest_blocks={},__fest_params,__fest_log_error,__fest_pushstr,' +
                '__fest_htmlchars=/[&<>\\"]/g,' +
                '__fest_htmlhash=' + JSON.stringify(__fest_htmlhash) + ',' +
                '__fest_jschars=/[\\\'\\"\\/\\n\\t\\b\\f]/g,' +
                '__fest_jshash=' + JSON.stringify(__fest_jshash) + ';' +
                'if(typeof __fest_push === "undefined"){__fest_pushstr = function(str){__fest_str+=str}}' +
                'if(typeof __fest_error === "undefined"){__fest_log_error = function(){return console.error.apply(console,arguments);};}else{__fest_log_error=__fest_error}' +
