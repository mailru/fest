var compile = (function(){
	...
	function _compile (file, options, output){
		...
		parser.onclosetag = function () {
			...
			switch (node.local){
				...
				case 'param':
					...
					return;
				case 'get':
					if (stack.indexOf('set') !== -1 || stack.indexOf('param') !== -1) {
						section.source += '__fest_fn=__fest_blocks[__fest_select];';
						section.source += 'if (__fest_fn)' + (
							options.mode === 'array' ? '__fest_buf.push(__fest_call(__fest_fn,__fest_params));' : '__fest_buf+=__fest_call(__fest_fn,__fest_params);'
						);
					} else {
		...
		var evallist = ['value', 'script', 'get', 'params'];
		...
		function build_template(output) {
			...
			template =
				'function ' + name + '(__fest_context){' +
					...
					'function __fest_call(fn, params){' +
						'for(var i in params)if(typeof params[i]=="function"&&params[i].param)params[i]=params[i]();' +
					...
					'if (__fest_to) {' +
						...
						'for (;__fest_iterator<__fest_to;__fest_iterator++) {' +
							...
							'} else {' +
								...
								'if (__fest_fn) ' +
									(options.mode === 'array' ? '__fest_html.push(__fest_call(__fest_fn,__fest_chunk.params));' : '__fest_html+=__fest_call(__fest_fn,__fest_chunk.params);') +
							'}' +
						'}' +
...
bash-3.2$ node benchmark/lebowski.js
fest x 41,886 ops/sec ±1.90% (94 runs sampled)
fest.content: 8.213 KB

fest.block x 29,826 ops/sec ±2.07% (88 runs sampled)
fest.content: 8.213 KB

NEW WAY:

bash-3.2$ node benchmark/lebowski.js
fest x 42,353 ops/sec ±2.27% (92 runs sampled)
fest.content: 8.213 KB

fest.block x 27,638 ops/sec ±2.41% (89 runs sampled)
fest.content: 8.213 KB
