var compile = (function(){
    ...
    function errorMessage(msg, badLine, file, offset) {
        function zeroPadding(s, len) {
            if (s.toString().length >= len) return s + "";
            return String(new Array(len + 1).join('0') + s).slice(-len);
        }
        
        function numSort(a, b) { return a - b; }
        
        function leftWhitespace(s) { return s.length - s.trimLeft().length; }
        ...
        var longest = num.sort(numSort)[num.length - 1].toString().length,
            minWhitespace = num.slice(0).map(function(n) { return leftWhitespace(lines[n]) }).sort(numSort)[0];
        
        num.forEach(function(n) {
            badPlace.push(('%n%: ' + lines[n].slice(minWhitespace)).replace('%n%', zeroPadding(n + 1, longest)));
        });
    ...
    function getAttr(compile_file, parser) {        
        return function(node, attr) {    
            try {
                ...
                throw new Error(file + "\n" + errorMessage('attribute "' + attr + '" is missing', parser.line, compile_file));
            }
            return attr;
        }
    }
    ...
    function finish_compile(file, options, name){
        ...
        try {
            ...
            log(e.message);
            template = 'function() { return "' + escape(e.message) + '"; }';
        }
