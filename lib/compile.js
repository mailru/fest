...
function compileAttributes(attrs){
    ...
    for (i in attrs){
        ...
        if (value.indexOf('fest') === 0){
            result.push({name: i, fest: attrs[i].value.replace(/^fest\:/, '')});
            result.push('');
        } else {
    ...
    if (attrs.some(function(item){return typeof item !== 'string';})){
        ...
        attrs.forEach(function(attr){
            if (typeof attr === 'string'){
                ...
                result += 'try{__fest_if=' + attr.fest + '}catch(e){__fest_if=false;__fest_error(e.message);}';
                result += 'if(__fest_if !== false){';
                result += '__fest_str+=" ' + attr.name + '=\\"";';
                result += 'try{__fest_str+=__fest_escapeHTML(__fest_if);}catch(e){__fest_error(e.message);}';
                result += '__fest_str+="\\"";';
                result += '}';
            }
        });
