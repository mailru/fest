var compile = (function(){
    ...
    function _compile (file, context_name, callbacks, strict, options, cb){
        ...
        parser.onopentag = function (node) {
            ...
            switch (node.local){
                ...
                case 'script':
                    ...
                    return;
                case 'insert':
                    fn[fn.length - 1] += '__fest_str+="' + readFileSync(dirname(file) + '/' + node.attributes.src.value, 'utf-8').replace(/"/g, '\\"').replace(/\n/g, '\\n').replace(/\r/g, '') + '";';
                    return;
                case 'for':
        ...
        parser.onclosetag = function () {
            ...
            if (inIgnored && node.local == 'attributes') {
                inIgnored = false;
                return;
            }            
            if (inIgnored) return;
