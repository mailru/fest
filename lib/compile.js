var compile = (function(){
	...
	function push_debug_info(section, parser, compile_file, block, debug){
		...
		section.source += '__fest_debug_block="' + block + '";;'; // second ; to prevent ";__fest_buf+=" collapsing
	}
	...
	function _compile (file, options, output){
		...
		function pushstr_begin(s) {
			if (options.async) {
				return '__fest_pushstr(' + (s ? s : '');
			} else {
				return '__fest_buf+=' + (s ? s : '');
			}
		}
		function pushstr_end(s) {
			if (options.async) {
				return (s ? s : '') + ');';
			} else {
				return (s ? s : '') + ';';
		...
		parser.onopentag = function (node) {
			...
			if (node.ns[node.prefix] != fest_ns){
				...
				opentag = true;
				section.source += pushstr('"<' + node.name + attrs.text + '"');
				return;
			}
			...
			switch (node.local){
				case 'element':
					...
					section.source += pushstr('"<" + __fest_element');
					return;
				case 'template':
					...
					return;
				case 'doctype':
					section.source += pushstr('"<!DOCTYPE "');
					return;
				case 'attribute':
					section.source += pushstr('" ' + escapeJS(_getAttr(node, 'name')) + '=\\""');
					return;
				case 'comment':
					section.source += pushstr('"<!--"');
					return;
				case 'cdata':
					section.source += pushstr('"<![CDATA["');
					return;
				case 'text':
					section.source += pushstr(node.attributes.value ? '"' + escapeJS(_getAttr(node, 'value')) + '"' : '');
					return;
				case 'space':
					section.source += pushstr('" "');
					return;
				...
				case 'value':
					...
					if (node.__fest_output === 'text'){
						section.source += pushstr_begin();
					} else if(node.__fest_output === 'html' && stack.indexOf('html:script') != -1) {
						section.source += pushstr_begin('__fest_escapeJS(');
					} else if(node.__fest_output === 'js') {
						section.source += pushstr_begin('__fest_escapeJS(');
					} else {
						section.source += pushstr_begin('__fest_escapeHTML(');
					}
					return;
				case 'script':
					if (!node.attributes.safe){
					...
					return;
				case 'call':
					section.source += 'try{' + pushstr(_getAttr(node, 'name', 'expr') + '(' + _getAttr(node, 'data', 'expr') + ')') + '}catch(e){__fest_log_error(e.message)}';
					return;
				case 'insert':
					section.source += pushstr('"' + escapeJS(readFileSync(dirname(file) + '/' + _getAttr(node, 'src'), 'utf-8')) + '"');
					return;
				case 'each':
		...
		parser.onclosetag = function () {
			...
			if (node.ns[node.prefix] != fest_ns){
				...
				if (!(node.name in short_tags)){
					section.source += pushstr('"</' + node.name + '>"');
				}
				return;
			}
			switch (node.local){
				case 'element':
					section.source += '__fest_element = __fest_element_stack[__fest_element_stack.length - 1];';
					section.source += 'if(!(__fest_element in __fest_short_tags)){' + pushstr('"</" + __fest_element + ">"') + '}';
				...
				case 'value':
					...
					if (!node.attributes.safe){
						section.source += pushstr_end() + '}catch(e){__fest_log_error(e.message + "' + this.line + '");}';
					} else {
						section.source += pushstr_end();
					}
					return;
				case 'set':
				...
				case 'get':
					if (stack.indexOf('set') !== -1) {
						...
						section.source += 'if (__fest_fn)__fest_buf+=__fest_fn.call(__fest_self,__fest_params);';
					} else {
		...
		parser.ontext = function (text) {
			...
			if (evallist.indexOf(stack[stack.length - 1]) === -1){
				section.source += pushstr('"' + escapeJS(text) + '"');
		...
		parser.oncdata = function (cdata) {
			...
			if (evallist.indexOf(stack[stack.length - 1]) === -1){
				section.source += pushstr('"' + escapeJS(cdata) + '"');
			} else if (stack[stack.length - 1] === 'get') {
	...
	function finish_compile(file, options, name){
		...
		function build_template(output) {
			...
			if (options.async) {
				...
				source = source.replace(/"\;__fest_buf\+=\"/g, '');
			}
			...
			template =
				'function ' + name + '(__fest_context){' +
					...
					(options.async ? 'function __fest_pushstr(s){__fest_buf+=s}' : '') +
					'if(typeof __fest_error==="undefined"){var __fest_error=function(){console.error.apply(console,arguments)}}' + // TODO test
					'function __fest_log_error(msg){__fest_error(msg+"\\nin block \\""+__fest_debug_block+"\\" at line: "+__fest_debug_line+"\\nfile: "+__fest_debug_file)}' + // TODO remove debug info for production code
					...
					'if (__fest_to) {' +
						...
						'for (; __fest_iterator < __fest_to; __fest_iterator++) {' +
							...
							'} else {' +
								...
								'if (__fest_fn) __fest_html += __fest_fn.call(__fest_self, __fest_chunk.params);' +
							'}' +
						'}' +
