		...
		parser.onopentag = function (node) {
			...
			if (node.ns[node.prefix] != fest_ns){
				...
				for (var i = 0; i < attrs.expr.length; i++) {
					if (vdom) {
						section.source += 'try{__fest_attrs[' + i + ']=__fest_string(' + _getExpr(attrs.expr[i], 'expression #' + (i + 1) + ' in attribute "' + attrs.name[i] + '"') + ')}catch(e){__fest_attrs[' + i + ']=""; __fest_log_error(e.message);}';
					} else {
		...
		function build_template(output) {
			...
			template =
				'function ' + name + '(__fest_context){' +
					...
					'function __fest_string(v){return v == null ? "" : v + "";}' +
