var compile = (function(){
	...
	function _compile (data){
		...
		function newVScope(local) {
			...
			return 'var __v0 = {attrs: {}, children: []};';
		}
		...
		parser.onopentag = function (node) {
			...
			if (node.ns[node.prefix] != fest_ns){
				...
				for (var i = 0; i < attrs.expr.length; i++) {
					if (vdom) {
						section.source += 'try{__fest_attrs[' + i + ']=(' + _getExpr(attrs.expr[i], 'expression #' + (i + 1) + ' in attribute "' + attrs.name[i] + '"') + ')}catch(e){__fest_attrs[' + i + ']=""; __fest_log_error(e.message);}';
					} else {
			...
			switch (node.local){
				...
				case 'value':
					...
					if (vdom) {
						...
						} else if (node.__fest_output === 'text' && !isAttributes) {
							section.source += '({tag:"<",children:';
						} else {
			...
			switch (node.local){
				...
				case 'value':
					if (vdom) {
						...
						if (node.__fest_output === 'text' && !isAttributes) {
							section.source += '}';
						}
					}
				...
				case 'get':
					if (vdom) {
						...
						if (should_inline_get_blocks()) {
							const expr = `__fest_call(__fest_fn,__fest_params,${(node.__params ? 'true' : 'false')})`;

							section.source += '__fest_fn = __fest_blocks[__fest_select];';
							section.source += 'if (__fest_fn) {';

							if (isAttributes) {
								section.source += `__fest_chunk = ${expr};`;
								section.source += `for (var __attr in __fest_chunk.attrs) {`;
								section.source += `  __v${vdomVar}.attrs[__attr] = __fest_chunk.attrs[__attr]`;
								section.source += `}`;
							} else {
