	...
	function _compile (data){
		...
		parser.onopentag = function (node) {
			...
			switch (node.local){
				...
				case 'include':
					...
					_compile({file: dirname(file) + '/' + _getAttr(node, 'src'), context_name:context_name, callbacks:callbacks, options:options, output:output});
					section = flush();
		...
		parser.onclosetag = function () {
			...
			switch (node.local){
				...
				case 'template':
					...
					return;
				case 'plural':
					section.source += '__fest_pushstr(__fest_context,__fest_params[__fest_plural(__fest_select)]);';
					return;
			}
		};
		var evallist = ['value', 'script', 'get', 'plural'];
		parser.ontext = function (text) {
			...
			if (evallist.indexOf(stack[stack.length - 1]) === -1){
				...
				section.source += '__fest_params = ["' + text.split('|').map(escapeJS).join('","') + '"];';
			} else {
		...
		parser.oncdata = function (cdata) {
			...
			if (evallist.indexOf(stack[stack.length - 1]) === -1){
				...
				section.source += '__fest_params = ["' + cdata.split('|').map(escapeJS).join('","') + '"];';
			} else {
