	...
	function _compile (data){
		...
		parser.onopentag = function (node) {
			...
			switch (node.local){
				...
				case 'include':
					...
					_compile({file: dirname(file) + '/' + _getAttr(node, 'src'), context_name:context_name, options:options, output:output});
					section = flush();
		...
		parser.onclosetag = function () {
			...
			switch (node.local){
				...
				case 'template':
					...
					return;
				case 'plural':
					section.source += '__fest_pushstr(__fest_context,__fest_params[__fest_plural(__fest_select)].replace(/%s/g, __fest_select));';
					return;
			}
		};
		...
		parser.ontext = parser.oncdata = function (text) {
			...
			if (stack[stack.length - 1] === 'script') {
				...
				// var escaped = text.replace(/%%/g, '%').replace.replace(/\\\|/g, "__VERTICAL_BAR__");
				// var plurals = escaped.split('|').map(function (chunk) {
				// return chunk.replace(/__VERTICAL_BAR__/g, '|');
				// });
				var plurals = text.replace(/%%/g, '%').match(/(\\.|[^|])+/g).map(function (chunk) {
			...
			template =
				'function ' + name + '(__fest_context){' +
					...
					'function __fest_escapeHTML(s){' +
						...
						'} else if (typeof s==="undefined")return "";' +
						'return s;}' +
					(output.plural && options.plural ? 'var __fest_plural=' + options.plural.toString() + ';' : '') +
					source +
