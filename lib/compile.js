        ...
        parser.onopentag = function (node) {
            ...
            switch (node.local){
                case 'template':
                    ...
                    context_name = (node.attributes.context_name ? getAttr(node, 'context_name') : context_name);
                ...
                case 'doctype':
                    ...
                    return;
                case 'attribute':
                    fn[fn.length - 1] += '__fest_str+=" ' + getAttr(node, 'name') + '=\\"";';
                    return;
                case 'comment':
                    fn[fn.length - 1] += '__fest_str+="<!--";';
                    return;
                case 'cdata':
                    ...
                    return;
                case 'text':
                    fn[fn.length - 1] += node.attributes.value ? '__fest_str+="' + getAttr(node, 'value') + '";' : '';
                    return;
                case 'space':
                    fn[fn.length - 1] += '__fest_str+=" ";';
                    return;
                case 'if':
                    fn[fn.length - 1] += 'try{__fest_if=' + getAttr(node, 'test') + '}catch(e){__fest_if=false;__fest_error(e.message);}';
                ...
                case 'script':
                    ...
                    return;
                case 'insert':
                    fn[fn.length - 1] += '__fest_str+="' + readFileSync(dirname(file) + '/' + getAttr(node, 'src'), 'utf-8')
                        .replace(/\\/g, '\\\\')
                        .replace(/"/g, '\\"')
                        .replace(/\n/g, '\\n')
                        .replace(/\r/g, '') + '";';
                    return;
                case 'for':
                    fn[fn.length - 1] += 'var ' + getAttr(node, 'index') + ';';
                    fn[fn.length - 1] += 'try{__fest_foreach=' + getAttr(node, 'iterate') + ' || {};}catch(e){__fest_foreach={};__fest_error(e.message);}';
                    fn[fn.length - 1] += 'for(' + getAttr(node, 'index') + ' in __fest_foreach){';
                    return;
                case 'foreach':
                    fn[fn.length - 1] += 'var ' + getAttr(node, 'index') + ',__fest_l' + counters.counter + ',__fest_from' + counters.counter + ',__fest_to' + counters.counter + ';';
                    if (node.attributes.iterate){
                        ...
                        fn[fn.length - 1] += 'for(' + getAttr(node, 'index') + ' = 0;' + getAttr(node, 'index') + '<__fest_l' + counters.counter + ';' + node.attributes.index.value + '++){';
                    } else {
                        ...
                        fn[fn.length - 1] += 'try{__fest_to' + counters.counter + '=' + getAttr(node, 'to') + ';}catch(e){__fest_to' + counters.counter + '=0;__fest_error(e.message);}';
                        fn[fn.length - 1] += 'for(' + getAttr(node, 'index') + ' = __fest_from' + counters.counter + ';' + getAttr(node, 'index') + '<=__fest_to' + counters.counter + ';' + getAttr(node, 'index') + '++){';
                    }
                    counters.counter++;
                    return;
                case 'set':
                    ...
                    fn[fn.length - 1] += 'if(__fest_if){';
                    fn[fn.length - 1] += '__fest_blocks' + getName(getAttr(node, 'name')) + '=function(' + context_name + ',params){';
                ...
                case 'get':
                    ...
                    fn[fn.length - 1] += 'return function(){return ("' + getAttr(node, 'name') + '" in __fest_blocks) ? __fest_blocks' + getName(getAttr(node, 'name')) + '(' + context_name + ', params) : [];};';
                    fn[fn.length - 1] += '};';
                ...
                case 'include':
                    if (node.attributes.context){
                        ...
                        fn[fn.length - 1] += 'try{' + context_name + '=' + getAttr(node, 'context') + '}catch(e){' + context_name + '={};__fest_error(e.message)};';
                    }
                    _compile(dirname(file) + '/' + getAttr(node, 'src'), context_name, callbacks, options,
    ...
    function finish_compile(file, options, name){
        ...
        if (options.xmllint) {
          validate(file, function(err) {
            if (err) {
              console.log(['[xmllint ' + file + ' fails]', err.message].join('\n'));
            } else {
              console.log('[xmllint ' + file + ' ok]');
            }
          });
        }
