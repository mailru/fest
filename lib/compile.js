var compile = (function(){
	...
	var CodeChunk = (new Function('module', readFileSync(__dirname + '/codechunk.js').toString() + '; return module.exports;'))({});
	var _tags = {};
	var __tagspath = __dirname + '/tags/';


	('fest:attribute fest:if fest:comment ' +
	'fest:doctype fest:cdata fest:value ' +
	'fest:for-each ' +
	'bem:block bem:mod bem:elem').split(' ').forEach(function (file) {
		var name = file.split('.')[0];
		_tags[name] = (new Function('module', readFileSync(__tagspath + name + '.js').toString() + '; return module.exports;'))({});
	});
	...
	function compileAttributes(attrs, node, stack, output){
		var name, result = { 'text': '', 'expr': [], 'name': [], 'code': '' }, n = 0, attrValue, chunk, retVal;

		for (name in attrs) {
			attrValue = attrs[name].value;

			if (_tags[name] && _tags[name].attr) {
				chunk = new CodeChunk;

				retVal = _tags[name].attr.call({
					tags: _tags,
					stack: stack,
					output: output
				}, chunk, attrValue, name, node, attrs);

				result.code += chunk.toString();
				result.text += chunk.getSource('attrs');
			...
			attrValue = attrValue.replace(/{{/g, "__DOUBLE_LEFT_CURLY_BRACES__").replace(/}}/g, "__DOUBLE_RIGHT_CURLY_BRACES__");
	...
	function _compile (data){
		...
		function closetag(name, opentag){
			...
			if (stack.indexOf('attributes') >= 0 || name === 'attributes' || name === 'mod'){
				return opentag;
			}
			...
			if (last === 'shorttag'){
				...
				if (stack.attrs) {
					stack.attrs.forEach(function (attr) {
						section.source += '__fest_pushstr(__fest_context," ' + attr.name +'=\\"" + ' + attr.value + ' + "\\"");';
					});
					stack.attrs = null;
				}
		...
		function _onTag(act, node, name) {
			if (_tags && _tags[arguments.length == 3 ? name : node.name]) {
				var chunk = new CodeChunk();

				_tags[name || node.name][act].call({
					tags: _tags,
					stack: stack,
					getAttr: _getAttr,
					escapeJS: escapeJS,
					output: output,
					compileExpr: function (value) {
						return compileExpr(value, _getExpr);
					}
				}, chunk, node, node.attributes);

				section.source += chunk.toString();
			}
		}
		...
		parser.onopentag = function (node) {
			...
			if (!node.ns[node.prefix] || node.ns[node.prefix].indexOf(fest_ns) == -1){
				attrs = compileAttributes(node.attributes, node, stack, output);
				section.source += attrs.code;
			...
			template =
				'function ' + name + '(__fest_context){' +
					...
					'var __fest_self=this,' +
						(options.additionalVars && (options.additionalVars + ',') || '') +
