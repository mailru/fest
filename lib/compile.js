var compile = (function(){
    ...
    if (fs !== null){
        var __read_file = fs.readFileSync;
    }
    if (path !== null){
    ...
    var sax = new Function(__read_file(__dirname + '/sax-js.js', 'utf-8') + '; return sax')();
    ...
    function _compile (file, context_name, callbacks, strict, options, cb){
            ...
            switch (node.local){
                ...
                case 'script':
                    ...
                    if (node.attributes.src){
                        fn[fn.length - 1] += __read_file(dirname(file) + '/' + node.attributes.src.value, 'utf-8');
                    }
                    return;
                case 'insert':
                    fn[fn.length - 1] += '__fest_str += "' + __read_file(dirname(file) + '/' + node.attributes.src.value, 'utf-8').replace(/\n/g, '\\n') + '";';
                    return;
                case 'for':
                ...
                case 'include':
                    ...
                    _compile(dirname(file) + '/' + node.attributes.src.value, context_name, callbacks, strict, options,
        ...
        parser.write(__read_file(file, 'utf-8')).close();
    }
    function finish_compile(file, strict, options, name){
        var callbacks = {}, template;
        name = name || '';

        _compile(file, '__fest_context', callbacks, strict || true, options || {trim:true, xmlns:true}, function(result){
            var __fest_jshash = {
                "\"":"\\\"",
                "\\":"\\\\",
                "/" :"\\/",
