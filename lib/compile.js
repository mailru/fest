        ...
        parser.onopentag = function (node) {
            ...
            switch (node.local){
                ...
                case 'for':
                    ...
                    return;
                case 'foreach':
                    fn[fn.length - 1] += 'var ' + node.attributes.index.value + ',__fest_l' + counters.counter + ',__fest_from' + counters.counter + ',__fest_to' + counters.counter + ';';
                    if (node.attributes.iterate){
                        ...
                        fn[fn.length - 1] += 'try{__fest_to' + counters.counter + '=' + node.attributes.to.value + ';}catch(e){__fest_to' + counters.counter + '=0;__fest_error(e.message);}';
                        fn[fn.length - 1] += 'for(' + node.attributes.index.value + ' = __fest_from' + counters.counter + ';' + node.attributes.index.value + '<=__fest_to' + counters.counter + ';' + node.attributes.index.value + '++){';
                    }
                    counters.counter++;
                    return;
                case 'set':
