var compile = (function(){
    var fs = require('fs'),
        path = require('path'),
        defer = new Function(fs.readFileSync(__dirname + '/defer.js', 'utf-8') + ' return defer')(),
        sax = new Function(fs.readFileSync(__dirname + '/sax-js.js', 'utf-8') + '; return sax')();
    ...
    function _compile (file, context_name, callbacks, strict, options){
        ...
        parser.onend = function () {
            promise.resolve(fn.join(''));
        };
        parser.write(fs.readFileSync(file, 'utf-8')).close();

        return promise;
    }
    function compile(file, strict, options, name){
        ...
        var result = _compile(file, '__fest_context', callbacks, strict || true, options || {trim:true, xmlns:true});
