var compile = (function(){
	...
	function closetag(stack, name, section, opentag){
		if (!opentag){
			return false;
		}
		...
		if (stack[stack.length - 1] === 'shorttag'){
			...
			section.source += '__fest_buf+=__fest_element in __fest_short_tags?"/>":">";';
		} else {
			section.source += '__fest_buf+=">";';
		}
		return false;
	...
	function _compile (file, options, output){
		...
		parser.onopentag = function (node) {
			...
			opentag = closetag(stack, node.local, section, opentag);
			...
			if (node.ns[node.prefix] != fest_ns){
				...
				opentag = true;
				section.source += '__fest_buf+="<' + node.name + attrs.text + '";';
				return;
			}
			...
			switch (node.local){
				case 'element':
					...
					section.source += '__fest_buf+="<" + __fest_element;';
					return;
				case 'template':
					...
					return;
				case 'doctype':
					section.source += '__fest_buf+="<!DOCTYPE ";';
					return;
				case 'attribute':
					section.source += '__fest_buf+=" ' + escapeJS(_getAttr(node, 'name')) + '=\\"";';
					return;
				case 'comment':
					section.source += '__fest_buf+="<!--";';
					return;
				case 'cdata':
					section.source += '__fest_buf+="<![CDATA[";';
					return;
				case 'text':
					section.source += node.attributes.value ? '__fest_buf+="' + escapeJS(_getAttr(node, 'value')) + '";' : '';
					return;
				case 'space':
					section.source += '__fest_buf+=" ";';
					return;
				...
				case 'value':
					...
					if (node.__fest_output === 'text'){
						...
						section.source += '__fest_buf+=__fest_escapeJS(';
					} else if(node.__fest_output === 'js') {
						section.source += '__fest_buf+=__fest_escapeJS(';
					} else {
						section.source += '__fest_buf+=__fest_escapeHTML(';
					}
					return;
				case 'script':
					if (!node.attributes.safe){
					...
					return;
				case 'call':
					section.source += 'try{__fest_buf+=' + _getAttr(node, 'name', 'expr') + '(' + _getAttr(node, 'data', 'expr') + ')}catch(e){__fest_log_error(e.message)}';
					return;
				case 'insert':
					section.source += '__fest_buf+="' + escapeJS(readFileSync(dirname(file) + '/' + _getAttr(node, 'src'), 'utf-8')) + '";';
					return;
				case 'each':
		...
		parser.onclosetag = function () {
			...
			if (node.ns[node.prefix] != fest_ns){
				...
				if (!(node.name in short_tags)){
					section.source += '__fest_buf+="</' + node.name + '>";';
				}
				return;
			}
			switch (node.local){
				case 'element':
					section.source += '__fest_element = __fest_element_stack[__fest_element_stack.length - 1];';
					section.source += 'if(!(__fest_element in __fest_short_tags)){__fest_buf+="</" + __fest_element + ">"}';
		...
		parser.ontext = function (text) {
			opentag = closetag(stack, 'text', section, opentag);
			if (evallist.indexOf(stack[stack.length - 1]) === -1){
				section.source += '__fest_buf+="' + escapeJS(text) + '";';
		...
		parser.oncdata = function (cdata) {
			opentag = closetag(stack, 'cdata', section, opentag);
			if (evallist.indexOf(stack[stack.length - 1]) === -1){
				section.source += '__fest_buf+="' + escapeJS(cdata) + '";';
			} else if (stack[stack.length - 1] === 'get') {
